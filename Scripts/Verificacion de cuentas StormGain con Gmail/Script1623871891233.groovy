...
for (def row = 88; row <= findTestData(dataTest).getRowNumbers(); row++) {
    ...
    new_email = findTestData(dataTest).getValue('new_email', row)
        ...
        try {
            ...
            if (!(error_loginPre)) {
                ...
                if (!(verified_badge_emailPre) && unverified_badge_emailPre) {
                    ...
                    WebUI.setText(sg.input_Email_email, new_email)
                    ...
                    boolean firstMailPre = false, firstMailPre1 = false;

                    boolean btn_emailPre = false

                    boolean a_verifyPre = false

                    boolean link_verifyPre = false
					...
					firstMailPre1 = WebUI.verifyElementPresent(gm.firtsMail_1, 1, FailureHandling.CONTINUE_ON_FAILURE)
                    ...
                    }else if(firstMailPre1) {
						WebUI.click(gm.firtsMail_1, FailureHandling.CONTINUE_ON_FAILURE)
						
						WebUI.delay(1)
						
						println(firstMailPre1)
					}
                    ...
                    if (!(verified_badge_emailPre) && unverified_badge_emailPre) {
                        ...
                        am.addAccountToCsv(csvPath, (('1,' + new_email) + ',') + password)
